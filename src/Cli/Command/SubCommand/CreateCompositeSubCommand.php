...
trait CreateCompositeSubCommand {
    /**
     * Create composite
     *
     * @subcommand create-composite
     * 
     * ## OPTIONS
     *
     * [<label>]
     ...
     *
     * [--class=<class>]
     * : The block classname, default is (Block/<labelToCamelCase>Composite)
     *
     ...
     */
    public function create_composite($args, $assoc_args)
    {
        $label = $args[0];
        $name = $assoc_args['name'] ?? sprintf('block.%s', static::strToKebabCase($args[0]));
        $class = $assoc_args['class'] ?? sprintf('Block/%sComposite', static::strToPascalCase($args[0]));
        ...
        ])->setBlockType(BlockType::Composite)->handle();
    }
}
